...
dataset <- dataset[dataset$taxOrder %in% c("Laminariales","Tilopteridales"),]
pointData <- dataset[,c("decimalLongitude","decimalLatitude")] 
colnames(pointData) <- c("Lon","Lat")
...
pointData <- pointData[!duplicated(pointData),]

## -----------------

titleMap <-  "Distribution of kelp forest records worldwide" 
subtitleMap <-  "Taxonomic orders Laminariales and Tilopteridales"
captionMap <- ""
fillLabel <- ""
...
Map1 <- ggplot() +
        ...
        geom_point(data = pointData, aes(x = Lon, y = Lat), color = "#9A3B04") + 
        coord_equal() +
