...
# Read oxygen layer
oxygenLayer <- raster("Data/DissolvedMolecularOxygen Surface Pred Mean.tif")

oxygenLayer <- aggregate(oxygenLayer, 12)
...
worldMap <- shapefile("Data/LandMass Polygon LR.shp")
...
plot <- ggplot() +
  geom_polygon(data = worldMap, aes(x = long, y = lat, group = group), fill="#FFFFFF", colour = "#FFFFFF" , size=0.25 ) +
  geom_tile(data = oxygenLayer, aes(x=Lon,y=Lat,fill=Val)) +
  scale_fill_gradientn(guide_legend(title="Sea surface dissolved oxygen (mmol.m3)"), colours=myColors, na.value='transparent') +
